...
<body class="container" id="body1">
	...
	<?php
    	...
    	if (isset($_POST['update'])) {
	    	...
	    	$hg_bereidingswijze = $_POST ['hg_bereidingswijze']; 

        	$query = "UPDATE gerechten SET hg_naam = :hg_naam, hg_chefnaam = :hg_chefnaam, hg_bereidingswijze = :hg_bereidingswijze WHERE g_id = :g_id";

	        $stmt = $db->prepare($query);
	        $stmt->bindValue(':g_id', $g_id);
	        $stmt->bindValue(':hg_naam', $hg_naam);
	        $stmt->bindValue(':hg_chefnaam', $hg_chefnaam);
	        $stmt->bindValue(':hg_bereidingswijze', $hg_bereidingswijze);
		...
		if (isset($_POST['delete'])) {
	        ...
	        $query = "DELETE FROM gerechten WHERE g_id = :g_id";
	        $stmt = $db->prepare($query);
	        $stmt->bindValue(':g_id', $g_id);
	        $stmt->execute();

	        echo "<div class='text-success'> Dit product is succesvol verwijdert!</div>";
	    }
		...
		if(isset($_POST['new'])) {
	        ...
	        if(empty($hg_naam) || empty($hg_chefnaam || $hg_bereidingswijze))  {
	        	echo "<div class='text-danger'>Alle velden zijn vereist !<div>";
	        } else {
		        $query = "INSERT INTO gerechten (g_id, hg_naam , hg_chefnaam, hg_bereidingswijze) VALUES (:g_id, :hg_naam, :hg_chefnaam, :hg_bereidingswijze)";
		        $stmt = $db->prepare($query);
		        $stmt->bindValue(':g_id', $g_id);
		        $stmt->bindValue(':hg_naam', $hg_naam);
		        $stmt->bindValue(':hg_chefnaam', $hg_chefnaam);
	...
	<?php
		...
		try {
			...
			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				?>
				<form action="muteer.php" method="post">
					<tr>
						...
						<td><input type="text" name="hg_bereidingswijze" value="<?php echo($row['hg_bereidingswijze']); ?>" maxlength="700"></td>
						<td><input type="submit" name="update" value="Update product"></td>
						<td><input type="submit" name="delete" value="Delete product" onclick="return confirm(' Bent u er zeker van om dit product te verwijderen?')"></td>
					</tr>
				</form>
			<?php
			}
			?>
				<form action="muteer.php" method="post">
					<tr>
						...
						<td><input type="text" name="hg_bereidingswijze" value="" maxlength="700" required></td>
		...
		<a class="dropdown-item" href="muteer.php">Product Wijzigen</a>
		</div>
